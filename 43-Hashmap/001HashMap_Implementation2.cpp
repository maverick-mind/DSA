...
public :
    HashMap(int N = 5 , double L = 0.7)
    {
        ...
        int & operator[](int key)
        {
           ...
           if(temp == NULL)
           {
                //key is not present in the hashMap, then insert it 
                int garbage ;
                insert(key , garbage); // now key exist in the hashMap
                temp = find(key);
...
int main() 
{
    ...
    // Hm.find(3);
    Hm.printHashMap() ;
    cout<<Hm[2]<<endl; // cout << Hm.operator[](2) << endl; for key : 2 , value : 4 
    cout<<Hm[3]<<endl; // 9 


    // Hm.operator[](2) = 100 ;
    // 4 = 100 ; // to fix this make operator overloading type as int& instead of int , int& will pass the refrence of 4 , and on that address , assign 4 

    //cout<<Hm[5]<<endl;
    // do Hm[key] only when you know key exists in the hashMap 
    // for ex : Hm[5] is undefined 

    Hm[2] = 100 ;  // refrence mein kuchh aur assign kar rhe hai 
    cout<<Hm[2]<<endl; // refrence ko print kar rhe hai 
    Hm.printHashMap() ;

    Hm[12] = 144 ; // if key value does not exist in hashMap , insert it 
    // Hm.operator[](12) = 144 ;
    Hm.printHashMap() ;
