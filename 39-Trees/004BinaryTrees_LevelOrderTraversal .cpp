...
// time : O(n) since each node is pushed, popped once from the queue
...
void printLevelOrder(TreeNode* root) {
	...
	while(!q.empty()) {
		...
		if(front->left != NULL) { // visit the left child of front node
			q.push(front->left);
		}
		
		if(front->right != NULL) { // visit the right child of front node
			q.push(front->right);
		}
	}
