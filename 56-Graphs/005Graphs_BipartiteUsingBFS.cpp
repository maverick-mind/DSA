...
bool bfs(int src, int color, const vector<vector<int>>& adj, vector<int>& colorMap) {
	...
	while (!q.empty()) {
		...
		for (int ngb : adj[cur]) {
			if (colorMap[ngb] == -1) {
				// ngb is not yet colored, color it
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		if (colorMap[i] == -1) {
			...
			// use bfs to check if the component of
			// node i is bipartite or not

			if (bfs(i, 0, adj, colorMap) == false) { // assume node i is given color 0
