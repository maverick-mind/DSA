...
int main() {
	...
	minHeap.insert({distMap[src], src}); // here , we are not inserting all the nodes with initial distances as infinity ,
	...
	while (!minHeap.empty()) {
		...
		for (auto [ngb, edgeWgt] : adj[cur]) {
			...
			if (!ex[ngb] and distMap[ngb] > distMap[cur] + edgeWgt) {
				...
				minHeap.erase({distMap[ngb], ngb}); // logV ; if {distMap[ngb], ngb} exists then it will be deleted , otherwise nothing will happen
